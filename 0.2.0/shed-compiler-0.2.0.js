            ...
            var currentItem = $shed.function(function() {
                return (function() {
                    ...
                    return sequences.isNil(itemOfSequences) ? $shed.memberAccess(sequences, sequences.nil) : (function() {
                        ...
                        return sequences.isNil(headSequence) ? concat(itemOfSequences.tail()).currentItem() : sequences.cons(headSequence.head(), ConcatSequence(sequence.cons(headSequence.tail(), itemOfSequences.tail())));
                    })();
                })();
            }).$define("currentItem");
            return {
...
$shed.exportModule("shed.compiler.parsing.modules", function() {
    ...
    var importRule = rules.sequence().next(tokenRules.keyword($shed.string("import"))).cut().capture(moduleName).capture(valuePath).next(tokenRules.symbol($shed.string(";"))).map($shed.memberAccess(nodes, nodes.importStatement));
    return {
