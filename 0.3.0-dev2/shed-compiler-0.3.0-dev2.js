    ...
    var extractNewline = $shed.function(function(input, indentationStack) {
        return (function() {
            ...
            return isSymbol(token, $shed.string("::")) ? sequences.head(input.tail()).map($shed.function(function(nextToken) {
                ...
                return indentationLevel.equals(currentIndentationLevel) ? withTail(sequences.singleton(tokens.newline(token.source()))) : (indentationLevel.lessThan(currentIndentationLevel) ? tuple(listOf(tokens.dedent(token.source()), tokens.newline(token.source())), input.tail(), pop(indentationStack)) : withTail($shed.memberAccess(sequences, sequences.nil)));
