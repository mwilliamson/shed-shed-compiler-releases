...
$shed.exportModule("shed.compiler.parsing.expressions", function() {
    ...
    var expressionRules = $shed.function(function(statementRule) {
        return (function() {
            ...
            var membersRule = rules.sequence().next(tokenRules.keyword($shed.string("members"))).cut().next(tokenRules.openingBrace()).capture(rules.zeroOrMoreWithSeparator(memberDeclarationRule, tokenRules.symbolOrNewline($shed.string(",")))).next(tokenRules.symbol($shed.string("}"))).next(rules.optional($shed.memberAccess(tokenRules, tokenRules.statementTerminator))).head();
...
$shed.exportModule("shed.compiler.tokenising.tokenFilter", function() {
    ...
    var extractNewline = $shed.function(function(input, indentationStack) {
        return (function() {
            ...
            return and(token.name().equals($shed.string("symbol")), token.value().equals($shed.string("{"))) ? sequences.head(input.tail()).map($shed.function(function(nextToken) {
                ...
                var isSignificantNewline = not(nextTokenIsComment(input.tail())) && regex.create($shed.string("\n( *)$")).exec(token.value()).map($shed.function(function(result) {
                    return result.capture($shed.number(1)).length().equals(sequences.head(indentationStack).valueOrElse($shed.function(function() {
    ...
    var nextTokenIsComment = $shed.function(function(input) {
        return sequences.head(input).map($shed.function(function(token) {
