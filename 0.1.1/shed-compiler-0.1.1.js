...
$shed.exportModule("lop.rules", function() {
    ...
    var token = $shed.function(function(name, value) {
        return $shed.function(function(tokens) {
            return withNextToken(tokens, $shed.function(function(token) {
                return token.name().equals(name) && token.value().equals(value) ? results.success(token.value(), tokens.tail()) : results.failure(unexpectedInputError(describeTokenNameAndValue(name, value, noSource), describeToken(token), token.source()));
            }));
        });
    }).$define("token");
    ...
    var withNextToken = $shed.function(function(tokens, func) {
        return sequences.head(tokens).map(func).valueOrElse($shed.function(function() {
            return results.failure(unexpectedInputError($shed.string("token"), $shed.string("end of token sequence"), noSource));
        }));
        ...
        return (function() {
            ...
            return $shed.function(function(tokens) {
                return rule(tokens).bindWithRemaining($shed.function(function(value, remaining) {
                    return (function() {
                        var source = createSource(tokens.head(), remaining.head());
