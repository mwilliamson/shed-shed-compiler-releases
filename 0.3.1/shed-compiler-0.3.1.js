...
$shed.exportModule("_sequenceItems", function() {
    var _hacks = $shed.js.import("_hacks");
    var Nil = (function() {
        var $class = $shed.class(function() {
            return {
                $class: $class,
                currentItem: $shed.function(function() {
                    return nil;
                })
            };
        });
...
$shed.exportModule("lazySequences", function() {
    ...
    var map = $shed.function(function(func, sequence) {
        return MappedSequence(func, sequence);
    }).$define("map");
    var MappedSequence = (function() {
        var $class = $shed.class(function(func, sequence) {
            var currentItem = $shed.function(function() {
                return (function() {
                    var item = sequence.currentItem();
                    return sequences.isNil(item) ? $shed.memberAccess(sequences, sequences.nil) : sequences.cons(func(item.head()), map(func, item.tail()));
                })();
            }).$define("currentItem");
            return {
                $class: $class,
        ...
        return $class;
    })().$define("MappedSequence");
    var filter = $shed.function(function(predicate, sequence) {
        return FilteredSequence(predicate, sequence);
    }).$define("filter");
    var FilteredSequence = (function() {
        var $class = $shed.class(function(predicate, sequence) {
            var currentItem = $shed.function(function() {
                return (function() {
                    var item = sequence.currentItem();
                    return sequences.isNil(item) ? $shed.memberAccess(sequences, sequences.nil) : (function() {
                        var head = item.head();
                        return predicate(head) ? sequences.cons(head, filter(predicate, item.tail())) : filter(predicate, item.tail()).currentItem();
                    })();
                })();
            }).$define("currentItem");
            return {
        ...
        return $class;
    })().$define("FilteredSequence");
    var concat = $shed.function(function(sequenceOfSequences) {
        return ConcatSequence(sequenceOfSequences);
    }).$define("concat");
    var ConcatSequence = (function() {
        var $class = $shed.class(function(sequenceOfSequences) {
            var currentItem = $shed.function(function() {
                return (function() {
                    var itemOfSequences = sequenceOfSequences.currentItem();
                    return sequences.isNil(itemOfSequences) ? $shed.memberAccess(sequences, sequences.nil) : (function() {
                        var headSequence = itemOfSequences.head().currentItem();
                        return sequences.isNil(headSequence) ? concat(itemOfSequences.tail()).currentItem() : sequences.cons(headSequence.head(), ConcatSequence(sequences.cons(headSequence.tail(), itemOfSequences.tail())));
                    })();
                })();
            }).$define("currentItem");
            return {
                ...
                tail: $shed.function(function() {
                    return concat(sequences.cons(headSequence.tail(), sequenceOfSequences.tail()));
                })
            };
        });
        return $class;
...
$shed.exportModule("strings", function() {
    ...
    var join = $shed.memberAccess(_strings, _strings.joinSequence);
    return {
        join: join
    };
});;
...
$shed.exportModule("sequences", function() {
    ...
    var lists = $shed.js.import("lists");
    var SequenceItem = $shed.memberAccess(_sequenceItems, _sequenceItems.SequenceItem);
    var nil = $shed.memberAccess(_sequenceItems, _sequenceItems.nil);
    var isNil = $shed.memberAccess(_sequenceItems, _sequenceItems.isNil);
    var cons = $shed.memberAccess(_sequenceItems, _sequenceItems.cons);
    var isEmpty = $shed.function(function(sequence) {
        return isNil(sequence.currentItem());
    }).$define("isEmpty");
    var head = $shed.function(function(sequence) {
        return (function() {
            var item = sequence.currentItem();
            return isNil(item) ? $shed.memberAccess(options, options.none) : options.some(item.head());
        })();
    }).$define("head");
    var tail = $shed.function(function(sequence) {
        return (function() {
            var item = sequence.currentItem();
            return isNil(item) ? $shed.memberAccess(options, options.none) : options.some(item.tail());
        })();
    }).$define("tail");
    var withItem = $shed.function(function(sequence, func) {
        return (function() {
            var item = sequence.currentItem();
            return isNil(item) ? $shed.memberAccess(options, options.none) : options.some(func(item.head(), item.tail()));
        })();
    }).$define("withItem");
    ...
    var anyTrampolined = $shed.function(function(predicate, sequence) {
        return (function() {
            var item = sequence.currentItem();
            return isNil(item) ? trampolining.stop(false) : (predicate(item.head()) ? trampolining.stop(true) : trampolining.nextFunction($shed.function(function() {
                return anyTrampolined(predicate, item.tail());
            })));
        })();
    ...
    var allTrampolined = $shed.function(function(predicate, sequence) {
        return (function() {
            var item = sequence.currentItem();
            return isNil(item) ? trampolining.stop(true) : (predicate(item.head()) ? trampolining.nextFunction($shed.function(function() {
                return allTrampolined(predicate, item.tail());
    ...
    var reversed2 = $shed.function(function(alreadyReversed, toReverse) {
        return (function() {
            var toReverseItem = toReverse.currentItem();
            return isNil(toReverseItem) ? alreadyReversed : reversed2(cons(toReverseItem.head(), alreadyReversed), toReverseItem.tail());
        })();
    }).$define("reversed2");
    var filter = $shed.function(function(predicate, sequence) {
        return lists.sequenceToList(sequence).filter(predicate);
    }).$define("filter");
    return {
        SequenceItem: SequenceItem,
        nil: nil,
    ...
    var none = (function() {
        var $class = $shed.class(function() {
            return {
                ...
                currentItem: $shed.function(function() {
                    return $shed.memberAccess(_sequenceItems, _sequenceItems.nil);
                }),
    ...
    var some = (function() {
        var $class = $shed.class(function(value) {
            return {
                ...
                currentItem: $shed.function(function() {
                    return _sequenceItems.cons(value, $shed.memberAccess(_sequenceItems, _sequenceItems.nil));
                }),
                equals: $shed.function(function(other) {
    ...
    var ListSet = (function() {
        var $class = $shed.class(function(list) {
            var contains = $shed.function(function(value) {
                return sequences.any($shed.function(function(element) {
    ...
    var AsValue = (function() {
        var $class = $shed.class(function(value, type, attributes) {
            var equals = $shed.function(function(other) {
                return sequences.all($shed.function(function(attribute) {
...
$shed.exportModule("structs", function() {
    ...
    var Struct = (function() {
        var $class = $shed.class(function(type, fields) {
            var _represent = $shed.function(function() {
                return type.identifier().concat($shed.string("(")).concat(strings.join($shed.string(", "), lazySequences.map(represent, fields))).concat($shed.string(")"));
            }).$define("_represent");
            var equals = $shed.function(function(other) {
                return equal(type, other.type()) ? equalList(fields, other.fields()) : false;
            }).$define("equals");
            var equalList = $shed.function(function(first, second) {
                return not(equal(first.length(), second.length())) ? false : sequences.all(pack(equal), lists.zip(first, second));
            }).$define("equalList");
            return {
                $class: $class,
...
$shed.exportModule("shed.compiler.moduleCompilation", function() {
    var strings = $shed.js.import("strings");
    var console = $shed.js.import("console");
    ...
    var compileSourceToString = $shed.function(function(source) {
        return (function() {
            ...
            return result.valueOrElse($shed.function(function(failure) {
                return failure.messages().forEach($shed.memberAccess($shed.memberAccess(console, console.stderr), $shed.memberAccess(console, console.stderr).write));
            }));
        })();
...
$shed.exportModule("shed.compiler.tokenising.tokens", function() {
    ...
    var newline = $shed.function(function(source) {
        return Token($shed.string("newline"), $shed.string("\n"), source);
    }).$define("newline");
    var indent = $shed.function(function(source) {
        return Token($shed.string("indent"), $shed.string(""), source);
    }).$define("indent");
    var dedent = $shed.function(function(source) {
        return Token($shed.string("dedent"), $shed.string(""), source);
    }).$define("dedent");
...
$shed.exportModule("shed.compiler.tokenising.tokenFilter", function() {
    var sequences = $shed.js.import("sequences");
    var lazySequences = $shed.js.import("lazySequences");
    var lists = $shed.js.import("lists");
    ...
    }).$define("filterTokens2");
    var extractNewline = $shed.function(function(input, indentationStack) {
        return (function() {
            var token = input.head();
            var withTail = $shed.function(function(filtered) {
                return tuple(filtered, input.tail(), indentationStack);
            });
            return isSymbol(token, $shed.string("::")) ? sequences.head(input.tail()).map($shed.function(function(nextToken) {
    ...
    var nextTokenIsComment = $shed.function(function(input) {
        return nextTokenIs($shed.function(function(token) {
            return token.name().equals($shed.string("comment"));
        }), input);
    }).$define("nextTokenIsComment");
    var nextTokenIs = $shed.function(function(predicate, input) {
        return sequences.head(input).map(predicate).valueOrElse($shed.function(function() {
            return false;
        }));
    }).$define("nextTokenIs");
    var isSymbol = $shed.function(function(token, value) {
        return isTokenValue(token, $shed.string("symbol"), value);
    }).$define("isSymbol");
    var isTokenValue = $shed.function(function(token, type, value) {
        return token.name().equals(type) && token.value().equals(value);
    }).$define("isTokenValue");
    var currentIndentation = $shed.function(function(indentationStack) {
        return sequences.head(indentationStack).valueOrElse($shed.function(function() {
            return $shed.number(0);
        }));
    }).$define("currentIndentation");
    var pop = $shed.function(function(sequence) {
        return sequences.tail(sequence).valueOrElse($shed.function(function() {
            return $shed.memberAccess(sequences, sequences.nil);
        }));
    }).$define("pop");
    return {
...
$shed.exportModule("shed.compiler.tokenising.tokeniser", function() {
    ...
    var sequences = $shed.js.import("sequences");
    var map = $shed.js.import("lazySequences").map;
    ...
    var Tokeniser = (function() {
        var $class = $shed.class(function() {
            var keywords = sets.fromList(listOf($shed.string("true"), $shed.string("false"), $shed.string("return"), $shed.string("package"), $shed.string("import"), $shed.string("val"), $shed.string("var"), $shed.string("public"), $shed.string("object"), $shed.string("class"), $shed.string("interface"), $shed.string("if"), $shed.string("else"), $shed.string("while"), $shed.string("fun"), $shed.string("for"), $shed.string("def"), $shed.string("then"), $shed.string("do"), $shed.string("members"), $shed.string("module"), $shed.string("let"), $shed.string("in"), $shed.string("pass")));
            var symbols = sets.fromList(listOf($shed.string("=>"), $shed.string("->"), $shed.string("<:"), $shed.string("&&"), $shed.string("::"), $shed.string("`"), $shed.string("¬"), $shed.string("!"), $shed.string("£"), $shed.string("$"), $shed.string("%"), $shed.string("^"), $shed.string("&"), $shed.string("*"), $shed.string("("), $shed.string(")"), $shed.string("-"), $shed.string("="), $shed.string("+"), $shed.string("["), $shed.string("]"), $shed.string("{"), $shed.string("}"), $shed.string(";"), $shed.string(":"), $shed.string("'"), $shed.string("@"), $shed.string("#"), $shed.string("~"), $shed.string("<"), $shed.string(">"), $shed.string(","), $shed.string("."), $shed.string("/"), $shed.string("?"), $shed.string("\\"), $shed.string("|")));
            var tokenise = $shed.function(function(input) {
                return sequenceToList(tokeniseString(input));
            }).$define("tokenise");
            var TokenSequence = (function() {
                var $class = $shed.class(function(input) {
                    var currentItem = $shed.function(function() {
                        return (function() {
                            var length = input.asString().length();
                            return length.greaterThan($shed.number(0)) ? (function() {
                                var nextToken = readNextToken(input);
                                return sequences.cons(nextToken.token(), TokenSequence(nextToken.rest()));
                            })() : sequences.singleton(tokens.end(input.range($shed.number(0), $shed.number(0)))).currentItem();
                        })();
                    }).$define("currentItem");
                    return {
                        $class: $class,
    ...
    var expressionRules = $shed.function(function(statementRule) {
        return (function() {
            var ifThenElseRule = lazyFunction($shed.function(function() {
                return rules.sequence().next(tokenRules.keyword($shed.string("if"))).cut().capture(expressionRule).next(tokenRules.keyword($shed.string("then"))).capture(expressionRule).next(rules.optional(tokenRules.newline())).next(tokenRules.keyword($shed.string("else"))).capture(expressionRule).map($shed.memberAccess(nodes, nodes.ifThenElse));
            }));
            var bracketedRule = lazyFunction($shed.function(function() {
                return rules.sequence().next(tokenRules.symbol($shed.string("("))).capture(expressionRule).cut().next(tokenRules.closingParen()).head();
            }));
            ...
            var formalArgumentListRule = lazyFunction($shed.function(function() {
                return rules.sequence().next(tokenRules.symbol($shed.string("("))).capture(rules.zeroOrMoreWithSeparator(formalArgumentRule, tokenRules.symbol($shed.string(",")))).next(tokenRules.closingParen()).head();
            }));
            var formalArgumentRule = lazyFunction($shed.function(function() {
                return rules.sequence().capture(tokenRules.identifier()).cut().capture(rules.optional(typeSpecifierRule)).map($shed.memberAccess(nodes, nodes.formalArgument));
            }));
            ...
            var memberSeparator = tokenRules.symbolOrNewline($shed.string(","));
            var membersBlockRule = rules.sequence().capture(rules.zeroOrMoreWithSeparator(memberDeclarationRule, memberSeparator)).next(rules.optional(memberSeparator)).head();
            var blockRule = $shed.function(function(blockBodyRule) {
                return rules.sequence().next(tokenRules.symbol($shed.string("::"))).cut().next(tokenRules.indent()).capture(blockBodyRule).next(rules.optional(tokenRules.newline())).next(tokenRules.dedent()).head();
            }).$define("blockRule");
            var membersRule = rules.sequence().next(tokenRules.keyword($shed.string("members"))).cut().capture(blockRule(membersBlockRule)).next(rules.optional($shed.memberAccess(tokenRules, tokenRules.statementTerminator))).head();
            ...
            var objectBodyRule = blockRule(rules.sequence().capture(optionalMembersRule).capture(rules.zeroOrMore(statementRule)).map($shed.memberAccess(nodes, nodes.obj)));
            ...
            var doBlockRule = rules.sequence().next(tokenRules.keyword($shed.string("do"))).cut().capture(blockRule(rules.zeroOrMore(statementRule))).map($shed.memberAccess(nodes, nodes.doBlock));
            ...
            var partialCallRule = lazyFunction($shed.function(function() {
                return rules.sequence().next(tokenRules.symbol($shed.string("("))).cut().capture(rules.zeroOrMoreWithSeparator(expressionRule, tokenRules.symbol($shed.string(",")))).next(tokenRules.closingParen()).map($shed.function(function(args) {
...
$shed.exportModule("shed.compiler.parsing.tokenRules", function() {
    ...
    var newline = $shed.function(function() {
        return rules.tokenOfType($shed.string("newline"));
    }).$define("newline");
    var symbolOrNewline = $shed.function(function(symbolValue) {
        return rules.sequence().next(rules.firstOf($shed.string(";"), listOf(symbol(symbolValue), newline()))).next(rules.zeroOrMore(newline())).map($shed.function(function() {
            return $shed.unit;
        }));
    }).$define("symbolOrNewline");
    var indent = $shed.function(function() {
        return rules.tokenOfType($shed.string("indent"));
    }).$define("indent");
    var dedent = $shed.function(function() {
        return rules.tokenOfType($shed.string("dedent"));
    }).$define("dedent");
    var statementTerminator = symbolOrNewline($shed.string(";"));
    var closingParen = $shed.function(function() {
        return rules.sequence().next(rules.optional(newline())).next(symbol($shed.string(")"))).map($shed.function(function() {
            return $shed.unit;
        }));
    }).$define("closingParen");
    return {
        ...
        identifier: identifier,
        newline: newline,
        symbolOrNewline: symbolOrNewline,
        indent: indent,
        dedent: dedent,
...
$shed.exportModule("shed.compiler.parsing.parser", function() {
    ...
    var filterTokens = $shed.js.import("shed.compiler.tokenising.tokenFilter").filterTokens;
    var nodes = $shed.js.import("shed.compiler.nodes");
    var results = $shed.js.import("shed.compiler.results");
    ...
    var Parser = (function() {
        var $class = $shed.class(function() {
            ...
            var parse = $shed.function(function(rule, input) {
                return tokenise(input).map(rule).valueOrElse();
            }).$define("parse");
            var filteredParse = $shed.function(function(predicate) {
                return $shed.function(function(rule, input) {
                    return tokenise(input).map($shed.function(function(tokens) {
                        return tokens.filter(predicate);
                    })).map(rule).valueOrElse();
                });
            }).$define("filteredParse");
            var parseWithoutEndToken = filteredParse($shed.function(function(token) {
                return not(token.name().equals($shed.string("end")));
            }));
            var parseModule = $shed.function(function(input) {
                return (function() {
                    var parseResult = parse(parseRule(), input);
                    return parseResult.isSuccess() ? results.success(parseResult.value()) : results.failure(listOf($shed.string("Parse failed: ").concat(parseResult.message().toString())));
                })();
            }).$define("parseModule");
            ...
            var tokenise = $shed.function(function(input) {
                return filterTokens(tokeniser.tokenise(input));
            }).$define("tokenise");
            return {
                $class: $class,
                parse: parse,
...
$shed.exportModule("shed.compiler.parsing.statements", function() {
    ...
    var nodes = $shed.js.import("shed.compiler.nodes");
    var tokenRules = $shed.js.import("shed.compiler.parsing.tokenRules");
    var statementTerminator = $shed.js.import("shed.compiler.parsing.tokenRules").statementTerminator;
    var statementRule = $shed.function(function(expressionRule) {
        return (function() {
            ...
            var defRule = rules.sequence().next(tokenRules.keyword($shed.string("def"))).cut().capture(tokenRules.identifier()).capture(expressionRule).next(rules.optional(statementTerminator)).map($shed.memberAccess(nodes, nodes.definition));
...
$shed.exportModule("shed.compiler.parsing.modules", function() {
    ...
    var valuePath = rules.zeroOrMore(rules.sequence().next(tokenRules.symbol($shed.string("."))).capture(tokenRules.identifier()).head());
    var moduleDeclaration = rules.sequence().next(tokenRules.keyword($shed.string("module"))).cut().capture(moduleName).next(statementTerminator).head();
    var importRule = rules.sequence().next(tokenRules.keyword($shed.string("import"))).cut().capture(moduleName).capture(valuePath).next(statementTerminator).map($shed.memberAccess(nodes, nodes.importStatement));
    return {
    ...
    var Generator = (function() {
        var $class = $shed.class(function(generate) {
            ...
            var importNode = $shed.function(function(importNode) {
                return (function() {
                    var importName = sequences.isEmpty(importNode.valuePath()) ? importNode.moduleName().last() : importNode.valuePath().last();
                    var moduleExpression = js.call(js.propertyAccess(js.propertyAccess(js.ref($shed.string("$shed")), $shed.string("js")), $shed.string("import")), listOf(js.string(strings.join($shed.string("."), importNode.moduleName()))));
                    return js.varDeclaration(importName, importNode.valuePath().foldLeft(moduleExpression, $shed.memberAccess(js, js.propertyAccess)));
                })();
            }).$define("importNode");
...
$shed.exportModule("shed.compiler.scoping", function() {
    ...
    var typeApplicationScope = $shed.function(function(node) {
        return sameScope(listOf(node.callee()).concat(node.args()));
    }).$define("typeApplicationScope");
    var memberAccessScope = $shed.function(function(node) {
        return sameScope(listOf(node.left()));
    }).$define("memberAccessScope");
    var objectScope = $shed.function(function(node) {
        return (function() {
            var memberDeclarationScope = node.memberDeclarations().map($shed.function(function(memberDeclaration) {
                return memberDeclaration.value();
            }));
            return subScope(memberDeclarationScope.concat(node.statements()));
        })();
    }).$define("objectScope");
...
$shed.exportModule("shed.compiler.compilation", function() {
    ...
    var main = $shed.function(function(argv) {
        return (function() {
            ...
            var mainFunction = sequences.head(argv.filter($shed.function(function(arg) {
...
$shed.exportModule("shed.compiler.nodes", function() {
    ...
    var ImportNode = (function() {
        var $class = $shed.class(function(moduleName, valuePath) {
            return {
                $class: $class,
                moduleName: $shed.function(function() {
                    return moduleName;
                }),
                valuePath: $shed.function(function() {
                    return valuePath;
                }),
                struct: $shed.function(function() {
                    return structs.create(ImportNode, listOf(moduleName, valuePath));
                })
            };
        });
        return $class;
    ...
    var Parser = (function() {
        var $class = $shed.class(function(name, prefixRules, infixRules) {
            ...
            var infixRulesUntilInclusive = $shed.function(function(name) {
                return remainingInfixRulesUntil(name, infixRules, true);
            }).$define("infixRulesUntilInclusive");
            var infixRulesUntilExclusive = $shed.function(function(name) {
                return remainingInfixRulesUntil(name, infixRules, false);
            }).$define("infixRulesUntilExclusive");
            var remainingInfixRulesUntil = $shed.function(function(name, rules, inclusive) {
                return sequences.withItem(rules, $shed.function(function(rule, remainingRules) {
                    return equal(rule.name(), name) ? (inclusive ? sequences.singleton(rule) : $shed.memberAccess(sequences, sequences.nil)) : sequences.cons(rule, remainingInfixRulesUntil(name, remainingRules));
...
$shed.exportModule("lop.testing", function() {
    ...
    var Token = $shed.js.import("lop.token").Token;
    var results = $shed.js.import("lop.results");
    var sources = $shed.js.import("lop.sources");
    ...
    var isFailureWithMessage = $shed.function(function(message) {
        return (function() {
            var $class = $shed.class(function() {
                var describeSelf = $shed.function(function() {
                    return $shed.string("failure with message: ").concat(represent(message));
                }).$define("describeSelf");
                var matches = $shed.function(function(result) {
                    return matchesWithDescription(result).matches();
                }).$define("matches");
                ...
                var matchesWithDescription = $shed.function(function(result) {
                    return not(result.isFailure()) ? MatchResult(false, $shed.string("was not failure, was ").concat(represent(result))) : (not(equal(result.message(), message)) ? MatchResult(false, $shed.string("message was ").concat(represent(result.message()))) : MatchResult(true, $shed.string("")));
                }).$define("matchesWithDescription");
                return {
                    $class: $class,
                    describeSelf: describeSelf,
    ...
    }).$define("isFailureWithMessage");
    var isFatalWithMessage = $shed.function(function(message) {
        return duck.equalTo(results.fatal(message));
    }).$define("isFatalWithMessage");
    var isSuccess = $shed.function(function(matcher) {
        return (function() {
            var $class = $shed.class(function() {
                ...
                var matchesWithDescription = $shed.function(function(result) {
                    return result.isSuccess() ? (not(matcher.matches(result.value())) ? MatchResult(false, $shed.string("value didn't match: ").concat(matcher.describeMismatch(result.value()))) : (not(sequences.isNil(result.remaining().currentItem())) ? MatchResult(false, $shed.string("entire input was not consumed, remaining was: ").concat(strings.join($shed.string(", "), lazySequences.map(represent, result.remaining())))) : MatchResult(true, $shed.string("")))) : MatchResult(false, $shed.string("was not success, was ").concat(represent(result)));
                }).$define("matchesWithDescription");
                return {
                    $class: $class,
                    describeSelf: describeSelf,
    ...
    var hasSource = $shed.function(function(expectedSource) {
        return (function() {
            var $class = $shed.class(function() {
                var describeSelf = $shed.function(function() {
                    return $shed.string("hasSource(").concat(represent(expectedSource)).concat($shed.string(")"));
                }).$define("describeSelf");
                var matches = $shed.function(function(value) {
                    return matchesWithDescription(value).matches();
                }).$define("matches");
                var describeMismatch = $shed.function(function(value) {
                    return matchesWithDescription(value).mismatchDescription();
                }).$define("describeMismatch");
                var matchesWithDescription = $shed.function(function(value) {
                    return sources.tryGetSource(value).map($shed.memberAccess(duck.equalTo(expectedSource), duck.equalTo(expectedSource).matchesWithDescription)).valueOrElse($shed.function(function() {
                        return MatchResult(false, $shed.string("value had no source"));
                    }));
                }).$define("matchesWithDescription");
                return {
                    $class: $class,
    ...
    return {
        isFailure: isFailure,
        isFailureWithMessage: isFailureWithMessage,
        isFatalWithMessage: isFatalWithMessage,
        isSuccess: isSuccess,
        hasSource: hasSource
    };
});;

$shed.exportModule("lop.positions", function() {
    var options = $shed.js.import("options");
    var calculateStringCoordinates = $shed.function(function(string, index) {
        return string.indexOf($shed.string("\n")).map($shed.function(function(newLineIndex) {
            return newLineIndex.lessThan(index) ? (function() {
                var remainingIndex = newLineIndex.add($shed.number(1));
                var remaining = string.sliceFrom(remainingIndex);
                var coordinatesInRemaining = calculateStringCoordinates(remaining, index.subtract(remainingIndex));
                return StringCoordinates(coordinatesInRemaining.lineIndex().add($shed.number(1)), coordinatesInRemaining.characterIndex());
            })() : StringCoordinates($shed.number(0), index);
        })).valueOrElse($shed.function(function() {
            return StringCoordinates($shed.number(0), index);
        }));
    }).$define("calculateStringCoordinates");
    var StringCoordinates = (function() {
        var $class = $shed.class(function(lineIndex, characterIndex) {
            return {
                $class: $class,
                lineIndex: $shed.function(function() {
                    return lineIndex;
                }),
                characterIndex: $shed.function(function() {
                    return characterIndex;
                })
            };
        });
        return $class;
    })().$define("StringCoordinates");
    return {
        calculateStringCoordinates: calculateStringCoordinates
    };
});;

$shed.exportModule("lop.messages", function() {
    var structs = $shed.js.import("structs");
    var UnexpectedInputError = (function() {
        var $class = $shed.class(function(expected, actual, location) {
            return {
                $class: $class,
                ...
                location: $shed.function(function() {
                    return location;
                }),
                struct: $shed.function(function() {
                    return structs.create(UnexpectedInputError, listOf(expected, actual, location));
                }),
                toString: $shed.function(function() {
                    return $shed.string("expected ").concat(expected).concat($shed.string(" but got ")).concat(actual).concat($shed.string("\nSource: ")).concat(location.describe());
                })
            };
        });
        return $class;
    })().$define("UnexpectedInputError");
    return {
    ...
    var Range = (function() {
        var $class = $shed.class(function(myStart, myEnd) {
            return {
                ...
                struct: $shed.function(function() {
                    return structs.create(Range, listOf(myStart, myEnd));
                })
            };
        });
        return $class;
...
$shed.exportModule("lop.results", function() {
    ...
    var failure = $shed.function(function(message) {
        return Failure(message, false);
    }).$define("failure");
    var fatal = $shed.function(function(message) {
        return Failure(message, true);
    }).$define("fatal");
    var Failure = (function() {
        var $class = $shed.class(function(message, isFatal) {
            return {
                $class: $class,
                map: $shed.function(function() {
                    return Failure(message, isFatal);
                }),
                ...
                message: $shed.function(function() {
                    return message;
                }),
                struct: $shed.function(function() {
                    return structs.create(Failure, listOf(message, isFatal));
                })
            };
        });
        return $class;
...
$shed.exportModule("lop.sources", function() {
    var structs = $shed.js.import("structs");
    var meta = $shed.js.import("meta");
    var Range = $shed.js.import("lop.range").Range;
    var positions = $shed.js.import("lop.positions");
    var StringSource = (function() {
        var $class = $shed.class(function(string, description, myRange) {
            return {
                $class: $class,
                asString: $shed.function(function() {
                    return string.substring(myRange.start(), myRange.end());
                }),
                originalString: $shed.function(function() {
                    return string;
                }),
                ...
                range: $shed.function(function(rangeStart, rangeEnd) {
                    return StringSource(string, description, Range(myRange.start().add(rangeStart), myRange.start().add(rangeEnd)));
                }),
                sliceFrom: $shed.function(function(index) {
                    return StringSource(string, description, Range(myRange.start().add(index), myRange.end()));
                }),
                struct: $shed.function(function() {
                    return structs.create(StringSource, listOf(string, description, myRange));
                }),
                describe: $shed.function(function() {
                    return (function() {
                        var coordinates = positions.calculateStringCoordinates(string, myRange.start());
                        return description.concat($shed.string("\nLine number: ")).concat(coordinates.lineIndex().add($shed.number(1)).toString()).concat($shed.string("\nCharacter number: ")).concat(coordinates.characterIndex().add($shed.number(1)).toString());
                    })();
                })
            };
        });
    ...
    var sourceKey = meta.createKey(StringSource, $shed.string("source"));
    var tryGetSource = $shed.function(function(value) {
        return meta.tryGet(value, sourceKey);
    }).$define("tryGetSource");
    var setSource = $shed.function(function(value, source) {
        return meta.set(value, sourceKey, source);
    }).$define("setSource");
    var NoSource = (function() {
        var $class = $shed.class(function() {
            return {
                $class: $class,
                struct: $shed.function(function() {
                    return structs.create(NoSource, emptyList);
                }),
                describe: $shed.function(function() {
                    return $shed.string("(no source)");
                })
            };
        });
        return $class;
...
$shed.exportModule("lop.rules", function() {
    ...
    var results = $shed.js.import("lop.results");
    var unexpectedInputError = $shed.js.import("lop.messages").unexpectedInputError;
    var sources = $shed.js.import("lop.sources");
    var noSource = $shed.js.import("lop.sources").noSource;
    var Range = $shed.js.import("lop.range").Range;
    var Rule = (function() {
        var $class = $shed.class(function() {
            return {
    ...
    var firstOf = $shed.function(function(name, rules) {
        return $shed.function(function(tokens) {
            return (function() {
                var ruleResults = lazySequences.map($shed.function(function(rule) {
                    return rule(tokens);
                }), rules);
                var stops = lazySequences.filter($shed.function(function(result) {
                    return or(result.isSuccess(), result.isFatal());
                }), ruleResults);
                return sequences.head(stops).valueOrElse($shed.function(function() {
                    return withNextToken(tokens, $shed.function(function(token) {
                        return results.failure(unexpectedInputError(name, describeToken(token), token.source()));
                    }));
                }));
            })();
        });
    ...
    var tokenOfType = $shed.function(function(name) {
        return $shed.function(function(tokens) {
            return withNextToken(tokens, $shed.function(function(token, rest) {
    ...
    var token = $shed.function(function(name, value) {
        return $shed.function(function(tokens) {
            return withNextToken(tokens, $shed.function(function(token, rest) {
                return token.name().equals(name) && token.value().equals(value) ? results.success(token.value(), rest) : results.failure(unexpectedInputError(describeTokenNameAndValue(name, value, noSource), describeToken(token), token.source()));
            }));
        });
    }).$define("token");
    ...
    var withNextToken = $shed.function(function(tokens, func) {
        return (function() {
            var item = tokens.currentItem();
            return sequences.isNil(item) ? results.failure(unexpectedInputError($shed.string("token"), $shed.string("end of token sequence"), noSource)) : func(item.head(), item.tail());
        })();
                ...
                return $shed.function(function(tokens) {
                    return previousRule(tokens).bindWithRemaining($shed.function(function(previousValue, remaining) {
                        return (function() {
                            ...
                            return nextResult.isFailure() && hasCut ? results.fatal(nextResult.message()) : nextResult.map($shed.function(function(nextValue) {
    ...
    var addSource = $shed.function(function(rule) {
        return (function() {
            var createSource = $shed.function(function(from, to) {
                return (function() {
                    var fromSource = from.source();
                    var toSource = to.source();
                    return sources.StringSource(fromSource.originalString(), fromSource.description(), Range(fromSource.indexRange().start(), toSource.indexRange().start()));
                })();
            });
            return $shed.function(function(tokens) {
                return rule(tokens).bindWithRemaining($shed.function(function(value, remaining) {
                    return (function() {
                        var source = createSource(tokens.currentItem().head(), remaining.currentItem().head());
                        var valueWithSource = sources.setSource(value, source);
                        return results.success(valueWithSource, remaining);
                    })();
                }));
            });
        })();
    ...
    var IsList = (function() {
        var $class = $shed.class(function(matchers) {
            ...
            var matchesElementsWithDescription = $shed.function(function(other) {
                return (function() {
                    var matchResults = lazySequences.map(pack(execMatcher), lists.zip(listRange($shed.number(0), matchers.length()), matchers, other));
                    var mismatches = lazySequences.concat(matchResults);
                    return sequences.head(mismatches).map($shed.function(function(mismatch) {
...
$shed.exportModule("hat", function() {
    ...
    var assertEquals = $shed.function(function(expected, actual) {
        return expected.equals(actual) ? results.success() : (function() {
    ...
    var run = $shed.function(function(argv) {
        return (function() {
            var resultsPromises = argv.map($shed.function(function(testModuleName) {
                return (function() {
            ...
            promises.combineList(resultsPromises).map($shed.function(function(results) {
                return (function() {
                    var isSuccess = results.map($shed.function(function(result) {
                        return result.isSuccess();
                    })).foldLeft(true, and);
                    isSuccess ? (function() {
                        print(greenCode);
                        print($shed.string("Pass!\n"));
                        print(blackCode);
                    })() : (function() {
                        print(redCode);
                        print($shed.string("Fail\n"));
                        print(blackCode);
                    })();
                })();
            }));
