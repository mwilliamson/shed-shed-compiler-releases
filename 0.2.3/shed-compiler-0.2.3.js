...
$shed.exportModule("shed.compiler.parsing.expressions", function() {
    ...
    var expressionRules = $shed.function(function(statementRule) {
        return (function() {
            ...
            var membersRule = rules.sequence().next(tokenRules.keyword($shed.string("members"))).cut().next(tokenRules.openingBrace()).capture(rules.zeroOrMoreWithSeparator(memberDeclarationRule, memberSeparator)).next(rules.optional(memberSeparator)).next(tokenRules.symbol($shed.string("}"))).next(rules.optional($shed.memberAccess(tokenRules, tokenRules.statementTerminator))).head();
            ...
            var objectBodyRule = rules.sequence().next(tokenRules.openingBrace()).capture(optionalMembersRule).capture(rules.zeroOrMore(statementRule)).next(tokenRules.symbol($shed.string("}"))).map($shed.memberAccess(nodes, nodes.obj));
            ...
            var doBlockRule = rules.sequence().next(tokenRules.keyword($shed.string("do"))).cut().next(tokenRules.openingBrace()).capture(rules.zeroOrMore(statementRule)).next(tokenRules.symbol($shed.string("}"))).map($shed.memberAccess(nodes, nodes.doBlock));
            ...
            var implicitLetInRule = lazyFunction($shed.function(function() {
                return (function() {
                    var valRule = rules.sequence().capture(letDeclarationRule).next($shed.memberAccess(tokenRules, tokenRules.statementTerminator)).head();
                    return rules.sequence().capture(valRule).cut().capture(rules.zeroOrMore(valRule)).capture(expressionRule).map($shed.function(function(firstVal, furtherVals, expression) {
                        return (function() {
                            var vals = lists.sequenceToList(sequences.cons(firstVal, furtherVals));
                            return nodes.letIn(vals, expression);
                        })();
                    }));
                })();
            }));
            var primaryExpressionRule = rules.firstOf($shed.string("primary expression"), listOf($shed.memberAccess(literals, literals.literalRule), variableReferenceRule, ifThenElseRule, bracketedRule, functionRule, objectRule, classRule, doBlockRule, letInRule, implicitLetInRule));
...
$shed.exportModule("shed.compiler.parsing.tokenRules", function() {
    ...
    var openingBrace = $shed.function(function() {
        return rules.sequence().next(symbol($shed.string("{"))).next(rules.zeroOrMore(newline())).map($shed.function(function() {
            return $shed.unit;
        }));
    }).$define("openingBrace");
...
$shed.exportModule("shed.compiler.tokenising.tokenFilter", function() {
    ...
    var filterTokens = $shed.function(function(input) {
        return lists.sequenceToList(sequences.reversed(filterTokens2($shed.memberAccess(sequences, sequences.nil), input, $shed.memberAccess(sequences, sequences.nil))));
    }).$define("filterTokens");
    ...
    var extractNewline = $shed.function(function(input, indentationStack) {
        return (function() {
            ...
            return and(token.name().equals($shed.string("symbol")), token.value().equals($shed.string("{"))) ? (function() {
                var noSignificantWhitespace = $shed.function(function() {
                    return tuple(sequences.singleton(token), input.tail(), sequences.cons(currentIndentation(indentationStack), indentationStack));
                });
                return sequences.head(input.tail()).map($shed.function(function(nextToken) {
                    return nextToken.name().equals($shed.string("whitespace")) ? regex.create($shed.string("\n( *)$")).exec(nextToken.value()).map($shed.function(function(regexResult) {
                        return tuple(sequences.singleton(token), input.tail(), sequences.cons(regexResult.capture($shed.number(1)).length(), indentationStack));
                    })).valueOrElse(noSignificantWhitespace) : noSignificantWhitespace();
                })).valueOrElse(noSignificantWhitespace);
            })() : (and(token.name().equals($shed.string("symbol")), token.value().equals($shed.string("}"))) ? tuple(sequences.singleton(token), input.tail(), sequences.isEmpty(indentationStack) ? $shed.memberAccess(sequences, sequences.nil) : indentationStack.tail()) : (token.name().equals($shed.string("whitespace")) ? (function() {
                var isAtCorrectIndentation = isNewlineAtIndentation(token.value(), currentIndentation(indentationStack));
                var isSignificantNewline = or(not(nextTokenIsComment(input.tail())) && isAtCorrectIndentation, nextTokenIsClosingBrace(input.tail()) && regex.create($shed.string("\n")).test(token.value()));
                return isSignificantNewline ? withTail(sequences.singleton(tokens.newline(token.source()))) : withTail($shed.memberAccess(sequences, sequences.nil));
            })() : (token.name().equals($shed.string("comment")) ? withTail($shed.memberAccess(sequences, sequences.nil)) : withTail(sequences.singleton(token)))));
        })();
    ...
    var nextTokenIsClosingBrace = $shed.function(function(input) {
        return nextTokenIs($shed.function(function(token) {
            return token.name().equals($shed.string("symbol")) && token.value().equals($shed.string("}"));
        }), input);
    }).$define("nextTokenIsClosingBrace");
    ...
    var isNewlineAtIndentation = $shed.function(function(value, indentation) {
        return regex.create($shed.string("\n( *)$")).exec(value).map($shed.function(function(result) {
            return result.capture($shed.number(1)).length().equals(indentation);
        })).valueOrElse($shed.function(function() {
            return false;
        }));
    }).$define("isNewlineAtIndentation");
